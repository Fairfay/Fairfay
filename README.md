...
Python Backend-разработчик с 4.5+ годами коммерческого опыта в FinTech и enterprise-автоматизации.
Специализация - платёжная инфраструктура: интеграции с провайдерами, обработка транзакций, event-driven архитектура. Оптимизировал платёжные модули до p95 <200 мс при 1200-1600 RPS, проектировал event-driven платёжные шлюзы, внедрял паттерны надёжности (идемпотентность, retry, rollback). Стек: Python, FastAPI, Django, PostgreSQL, Kafka, Redis, Celery, Docker, Kubernetes. Ищу роль Python backend-разработчика в продуктовой команде с фокусом на платёжные системы, высоконагруженные API или event-driven архитектуру.

<details>
<summary><h2>Опыт работы 4.5+ года:</h2></summary>
  <h4>
  ...
  <div>
    ...
    <span>Май 2026 — По настоящее время</span>
  </div>
  <div>
      <br><strong>Разрабатываю и поддерживаю интеграции с внешними платёжными провайдерами в Wallet-направлении в команде из 7 backend-разработчиков: REST API, webhooks, обеспечение атомарности транзакций и синхронизации балансов.</strong>
      <br> - Оптимизировал ключевой платёжный модуль с объёмом операций более $100 000 в день: переработал горячие SQL/ORM-запросы, добавил необходимые индексы, вынес тяжелые выборки в Redis (cache-aside, TTL 10мин) и оптимизировал работу connection pool, снизив p95 latency с 420 мс до 170 мс при нагрузке 1200-1600 RPS.
      <br>- Внедрил retry с exponential backoff и rollback-механизмы, что позволило снизить количество инцидентов при взаимодействии с внешними провайдерами с 3 в день до 1 в неделю (замер за 1 месяц после внедрения).
      <br>- Реализовал безопасную валидацию и обработку webhook-событий, включая проверку цифровых подписей, идемпотентность и защиту от replay-атак.
      <br>- Обеспечил корректную синхронизацию балансов и защиту от повторного выполнения финансовых операций при сетевых сбоях и повторной доставке событий.
      <br>- Построил observability-стек (Prometheus + Grafana + Sentry + OpsGenie): настроил алерты на p95 latency и error rate - снизил MTTR с 2 дней до 10 минут.
      <br>- Участвую в code review 10-15 pull request в неделю, проверяю архитектурные решения, качество кода и потенциальные проблемы с производительностью.
      <br>Стек: Python, FastAPI, PostgreSQL, Elasticsearch, ClickHouse, Kafka, Redis, AsyncIO, Docker, Kubernetes, Pydantic, CI/CD, Prometheus, Grafana, Sentry, OpsGenie.
  </div>
  ...
  <div>
    ...
    <span>Июнь 2025 — Май 2026</span>
  </div>
  <div>
    <br><strong>Проект: Платёжный шлюз и система уведомлений для благотворительных платформ "Дом с маяком" и "Давай соберёмся"</strong>
    <br>- Спроектировал и реализовал event-driven платёжный шлюз с асинхронной обработкой платежей, c нагрузкой в 300rps в команде из 4 разработчиков.
    <br>- Перевёл обработку платёжных событий на асинхронный I/O (asyncio, asyncpg, redis.asyncio) и изолировал воркеры по типам нагрузки - снизил p95 обработки события с 3 с до <250 мс.
    <br>- Использовал multiprocessing для параллельной обработки CPU-bound задач и эффективной утилизации нескольких ядер, разгрузив основной event loop и увеличив пропускную способность фоновой обработки.
    <br>- Построил систему email/Telegram/webhook-уведомлений с гарантией доставки.
    <br>- Написал unit- и интеграционные тесты с покрытием более 92% (794теста), участвовал в проектировании микросервисной архитектуры и production-деплое.
    <br>Стек: Python, FastAPI, PostgreSQL, Redis, RabbitMQ, Celery, Kafka, AsyncIO, Docker, CI/CD, multiprocessing.
    <br><strong>Проект: Автоматизация электронного документооборота для "Маревен Фуд Сэнтрал".</strong>
    <br>- Реализовал интеграции с SAP, Diadoc и Контур API - обеспечил переход документооборота компании в полностью электронный формат.
    <br>- Автоматизировал обработку документов, существенно сократив время согласования с 1-2 рабочих дней до 2-4 часов, и снизив долю ручных операций на 70-80%
    <br>- Оптимизировал SQL-запросы, снизил время выполнения тяжёлых выборок с 30 секунд до 3с, после EXPLAIN ANALYZE, индексов и устранения N+1 сократил число SQL-запросов на один бизнес-сценарий с 120 до 18.
    <br>Стек: Python, Flask, MySQL, Redis, RabbitMQ, Celery, AsyncIO, Docker, CI/CD, REST API.
  </div>
  <h4>
  ...
  <div>
    ...
    <span>Апрель 2022 — Июнь 2025</span>
  </div>
  <div>
    <br><strong>Разработал с нуля корпоративную систему управления проектами Yourtask с поддержкой PWA/offline и real-time взаимодействия, в команде из 2 Backend-разработчиков.</strong>
    <br>- Совмещал full-time разработку с обучением в КузГТУ в сессионном формате (2022-2024).
    <br>- Интегрировал Yandex ID OAuth и Telegram-авторизацию, что повысило конверсию успешного прохождения авторизации с 52% до 76% в течение первого месяца после внедрения по данным ЯндексМетрики.
    <br>- Внедрил CI/CD, сократив время выката релизов с 30 до 5м
    <br>- Реализовал централизованный мониторинг ошибок с нотификацией в Telegram, что сократило среднее время реакции на критические ошибки с 25 до 5м.
    <br>- Достиг покрытия кода тестами более 97% (unit + интеграционные), использовались fixtures, parametrize, mocking.
    <br>- Продукт прошёл сертификацию как отечественное ПО и внедрён в муниципальных и коммерческих организациях Кузбасса.
    <br>Стек: Python, Django, React, PostgreSQL, Redis, AWS, Celery, AsyncIO, WebSocket, Docker, CI/CD, Sentry, PWA, agile.
  </div>
</details>
